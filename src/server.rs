...
mod tests
{
	...
	mod postgres
	{
		...
		async fn post() -> DynResult<()>
		{
			...
			let contact_ = client
				.test_post_success::<PgContact, _>(&pool, routes::CONTACT, &admin.0, &admin.1, contact_args())
				.await;
			...
			let department = client
				.test_post_success::<PgDepartment, _>(
					&pool,
					routes::DEPARTMENT,
					&admin.0,
					&admin.1,
			...
			let location = client
				.test_post_success::<PgLocation, _>(&pool, routes::LOCATION, &admin.0, &admin.1, location_args())
				.await;
...

			{
				...
				let expected = {
					...
					let row = PgExpenses::retrieve(&pool, Match::from_iter(entity.iter().map(|x| x.id)).into())
						.await
						.unwrap();
			...
			let role =
				client.test_post_success::<PgRole, _>(&pool, routes::ROLE, &admin.0, &admin.1, role_args()).await;
